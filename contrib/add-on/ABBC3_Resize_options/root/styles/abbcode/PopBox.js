...
function ImageResizerOn( ObjImage )
{
	...
	var objResizerDiv				= document.createElement( 'div' );
		...
		images_x_id = CreateRandomId();
		pbSrc[images_x_id] = new Image();
	
	if ( ObjImage.className == 'attach_parent' )
	{
		pbSrc[images_x_id].src = ObjImage.parentNode.href;
		ObjImage.parentNode.setAttribute( 'id', images_x_id );
		ObjImage.parentNode.setAttribute( 'pbSrc', ObjImage.parentNode.href );
	} else {
		pbSrc[images_x_id].src = ObjImage.src;
		ObjImage.setAttribute( 'id', images_x_id );
		ObjImage.setAttribute( 'pbSrc', ObjImage.src );
	}
	...
	for (var pia = 0; pia < posted_images_ary.length; pia++)
	{
		...
		switch ( img.className )
		{
			...
			case 'attach_parent':
				...
				{
					case 'AdvancedBox':
						img.setAttribute( 'pbSrc', img.parentNode.href );
						if ( !ImageResizer ) { img.onclick = function() { Pop( this, 50, 'PopBoxImageLarge' ); return false; }; }
						break;
					case 'enlarge':
			...
			case 'attach_me':
				...
				{
					case 'AdvancedBox':
						img.setAttribute( 'pbSrc', img.src );
						if ( !ImageResizer ) { img.onclick = function() { Pop( this, 50, 'PopBoxImageLarge' ); return false; }; }
						break;
					case 'enlarge':
