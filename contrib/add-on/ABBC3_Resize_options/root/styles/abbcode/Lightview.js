...
function ImgOnLoad()
{
	...
	for (var pia = 0; pia < posted_images_ary.length; pia++)
	{
		...
		if ( img.className.match(/(^|\s)(highslide|image-resize)(\s|$)/) ) { continue; }
			...
			switch (ImageResizerMode)
			{
				case 'AdvancedBox' :
					if ( !GetParent )
					{
						...
						img.onclick = function () { Lightview.show({ href: this.src, options: { width: 800, height: 600 }}); return false; }
					}
					else
					{
						img.className = ( img.className ? 'lightview ' + img.className : 'lightview' );
						img.setAttribute('rel', 'gallery[myset]'); img.rel = "gallery[myset]";
						img.parentNode.onclick = function () { Lightview.show({ href: this.href, options: { width: 800, height: 600 }}); return false; }
					}
					break;
				default:
				case 'newwindow':
