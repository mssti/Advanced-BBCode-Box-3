<?php
/**
* @package: phpBB 3.0.5 :: Advanced BBCode box 3 -> root/includes
* @version: $Id: abbcode.php, v 1.0.12 2009/08/01 09:08:01 leviatan21 Exp $
...
class abbcode
{
	...
	**/
	function anchor_pass($a_id, $a_href, $string)
	{
		...
		if ($a_href != '')
		{
			return str_replace(array('{ANCHOR_ID}','{ANCHOR_HREF}', '{ANCHOR_TEXT}'), array($a_id, $a_href, $string), $this->bbcode_tpl('anchor_link'));
		}
		// Isn't an anchor
		else
		{
	...
	* @version 1.0.11
	* 
	* Enlaces eD2k b�sicos					: ed2k://|file|>nombre del archivo<|>tama�o del archivo<|>hash del archivo<|/
	* Enlaces eD2k con conjunto de hashes	: ed2k://|file|>nombre del archivo<|>tama�o del archivo<|>hash del archivo<|p=>conjunto de hashes<|/
	* Enlaces eD2k con fuentes				: ed2k://|file|>nombre del archivo<|>tama�o del archivo<|>hash del archivo<|/|sources,>IP:puerto<|/
	* Enlace eD2k con host					: ed2k://|file|>nombre del archivo<|>tama�o del archivo<|>hash del archivo<|/|sources,>nombre del host:puerto<|/
	* Enlace eD2k en HTML					: <a href= "ed2k://|file|>archivo<|>tama�o del archivo<|>hash del archivo<|/">nombre mostrado en la p�gina web
	* Enlace eD2k con fuentes HTTP			: ed2k://|file|>nombre del archivo<|>tama�o del archivo<|>hash del archivo<|s=http://cualquierweb.net/archivo|/
	* Enlaces eD2k con Hash Ra�z			: ed2k://|file|>nombre de archivo<|>tama�o del archivo<|>hash del archivo<|h=>hash ra�z<|/
	**/
	...
	**/
	function spoil_pass($in)
	{
		...
		return str_replace(array('{SPOILER_SHOW}', '{SPOILER_HIDE}', '{SPOILER_TEXT}'), array("'" . $user->lang['SPOILER_SHOW'] . "'", "'" . $user->lang['SPOILER_HIDE'] . "'", $in), $this->bbcode_tpl('spoiler'));
	}
	...
	* @version 1.0.12
	...
	//	return '<div id="scrippet"><div class="scrippet-shadow"><div class="inner-shadow">' . $output . '<br /></div></div></div>';
	}
	...
	**/
	function testlink_pass($in)
	{
		...
		// If you want to hide links to visitors and bots, uncomment this lines
	//	if ($user->data['user_id'] == ANONYMOUS || $user->data['is_bot'])
	//	{
	...
	//	}
		...
		else
		{
			...
			$linktest_pic_wrong	= '<img src="' . $path . '/images/error.gif" class="postimage" alt="' . $user->lang['ABBC3_TESTLINK_WRONG'] . '" />&nbsp;';
			$linktest_msg_good	= '<span class="abbc3_good">' . $user->lang['ABBC3_TESTLINK_GOOD'] . '</span> ';
			$linktest_pic_good	= '<img src="' . $path . '/images/ok.gif"    class="postimage" alt="' . $user->lang['ABBC3_TESTLINK_GOOD'] .'" />&nbsp;';
			...
			foreach ($linktest_links as $linktest_value)
			{
				...
				//	$linktest_value		= trim(str_replace('\"', '',preg_replace('#<!-- ([lmwe]) --><a class=(.*?) href=(.*?) onclick=(.*?)>(.*?)</a><!-- ([lmwe]) -->#si','$3', trim($linktest_value))));
					...
					$linktest_value		= '<a href="' .$linktest_value . '" onclick="window.open(this.href);return false;" title="">' . $linktest_value . '</a>';
			...
			if (count($linktest_result) > 0)
			{
				...
				//	$linktest_echo .= $linktest_data['msg'] . $linktest_data['link'] . "<br />";
					$linktest_echo .= $linktest_data['pic'] . $linktest_data['link'] . "<br />";
				}
			}
	...
	**/
	...
	function donate_pass($support = '', $text = '')
	{
		...
		if ($support)
		{
			return '<a href="' . append_sid("{$phpbb_root_path}donate/index.$phpEx", "support=$support") . '">' . $user->img('button_donate', $user->lang['DONATE']) . (($text) ? '&nbsp;' . ($text) : '') . '</a>';
		}
		return '<a href="' . append_sid("{$phpbb_root_path}donate/index.$phpEx") . '">' . $user->img('button_donate', $user->lang['DONATE']) . (($text) ? '&nbsp;' . ($text) : '') . '</a>';
	}
...
// Not available yet
//			'bbc.co'			=> array ('display' => true	,'image' => 'bcc.gif'			,'example' => "http://news.bbc.co.uk/1/hi/business/7847055.stm"																					,'found' => "#http://news.bbc.co.uk/(.*?)/hi/(.*?)/([^[]*)?#sie"										,'regexp' => "\$this->external_video('\$0', 'object')"),
			...
			'filefront.com'		=> array ('display' => true	,'image' => 'filefront.gif'		,'example' => "http://files.filefront.com/Mulgore+Model+Edited/;12826486;/fileinfo.html"														,'found' => "#http://files.filefront.com/(.*?)/;(.*?);/fileinfo.html#sie"								,'regexp' => "\$this->external_video('\$0', 'object')"),
			'g4tv.com'			=> array ('display' => true	,'image' => 'g4tv.gif'			,'example' => "http://www.g4tv.com/xplay/videos/29265/Infamous_All_Access.html"																	,'found' => "#http://((.*?)?)g4tv.com/(.*?)/(.*?)/(.*?)/([^[]*)?#si"									,'regexp' => '<br />0=($0)<br />1=($1)<br />2=($2)<br />3=($3)4=($4)<br />5=($5)<br />6=($6)<object width="{WIDTH}" height="{HEIGHT}" id="VideoPlayer"><param name="movie" value="http://www.g4tv.com/lv3/$5" /><param name="allowScriptAccess" value="always" /><param name="allowFullScreen" value="true" /><embed src="http://www.g4tv.com/lv3/$5" type="application/x-shockwave-flash" name="VideoPlayer" width="{WIDTH}" height="{HEIGHT}" allowScriptAccess="always" allowFullScreen="true" /></object>'),
			...
			'gameprotv.com'		=> array ('display' => true	,'image' => 'gameprotv.gif'		,'example' => "http://www.gameprotv.com/left-4-dead-anlisis-video-5553.html"																	,'found' => "#http://www.gameprotv.com/(.*)-video-([0-9]+)?.([^[]*)?#is"								,'regexp' => '<object id="WMPlay" width={WIDTH} height={HEIGHT} classid="CLSID:6BF52A52-394A-11d3-B153-00C04F79FAA6" codebase="http://activex.microsoft.com/activex/controls/mplayer/en/nsmp2inf.cab#Version=5,1,52,701" standby="Loading Microsoft Windows Media Player components..." type="application/x-oleobject"><param name="url" value="http://www.gameprotv.com/0.php?id=$2"><param name="autoStart" value="0"/><param name="AnimationatStart" value="0" /><param name="showdisplay" value="1" /><param name="TransparentAtStart" value="0" /><param name="ShowControls" value="1" /><param name="ShowStatusBar" value="1" /><param name="ClickToPlay" value="0" /><param name="bgcolor" value="#000000" /><param name="volume" value="100%" /><param name="InvokeURLs" value="0" /><param name="loop" value="0" /><embed width="{WIDTH}" height="400" type="video/x-ms-wmv" pluginspage="http://www.microsoft.com/Windows/Downloads/Contents/Products/MediaPlayer/" src="http://www.gameprotv.com/0.php?id=$2" Name="GameproTV" ShowControls="1" AutoStart="0" ShowDisplay="0" ShowStatusBar="1"></embed></object>'),
...
//			'www.lala'			=> array ('display' => true	,'image' => 'lala.gif'			,'example' => "http://www.lala.com/#memberplaylist/2458P24912"																					,'found' => "#http://www.lala.com/\#memberplaylist/([^[]*)?#si"											,'regexp' => '<object type="application/x-shockwave-flash" data="http://www.lala.com/external/flash/PlaylistWidget.swf" id="lalaPlaylistEmbed" width="{WIDTH}" height="{HEIGHT}"><param name="movie" value="http://www.lala.com/external/flash/PlaylistWidget.swf"/><param name="wmode" value="transparent"/><param name="allowNetworking" value="all"/><param name="allowScriptAccess" value="always"/><param name="flashvars" value="playlistId=$1&host=www.lala.com&partnerId=memberplaylist"/><embed id="lalaPlaylistEmbed" name="lalaPlaylistEmbed" src="http://www.lala.com/external/flash/PlaylistWidget.swf" width="{WIDTH}" height="{HEIGHT}" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" wmode="transparent" allowNetworking="all" allowScriptAccess="always" flashvars="playlistId=$1&host=www.lala.com&partnerId=memberplaylist"></embed></object>'),
			...
			'slideshare.net'	=> array ('display' => true	,'image' => 'slideshare.gif'	,'example' => "http://www.slideshare.net/chrisbrogan/social-media-for-publishers-presentation"													,'found' => "#http://www.slideshare.net/(.*?)/([^[]*)?#sie"												,'regexp' => "\$this->external_video('\$0', 'embed')"),
...
// Fix for work with extra video data between /video/ and the /id
		...
		foreach ($this->abbcode_video_ary as $video_name => $video_data)
		{
			...
			if ((preg_match('#' . $video_name . '#si', $in)) && ($video_data['display']))
			{
				...
				if ($video_data['found'] != '' && $video_data['regexp'] != '')
				{
					...
					//	if (trim($video_content) == trim($in))
					//	{
					//		return '[BBvideo]' . $in . '[/BBvideo]' ;
					//	}

						// Resize acording the video settings
					...
					//	return "<br />" . $video_image . $video_link . "<br />" . $video_content . "<br />";
					}
				}
			}
		}
