...
function ImgOnLoad()
{
	...
	/** class ="attach_parent" phpbb thumbnail attached - Recomended true **/
	var exclude_thumbnail_attached	= true; // false; // true;

	/** class="attach_me" phpbb image attached - Recomended false **/
	...
	for (var pia = 0; pia < posted_images_ary.length; pia++)
	{
		...
		switch (ImageResizerMode)
		{
			case 'AdvancedBox' :
				...
				else if ( img.className == 'attach_parent' )
				{
				...
				else if ( img.className == 'attach_me' )
				{	
			...
			case 'newwindow':
				...
				{
					img.onclick = function() { return false; } // Do not touch, need it
					if ( !ImageResizer )
					{
...
{
//	alert( 'imgsrc=('+imgsrc+') imgwidth=('+imgwidth+') imgheight=('+imgheight+')' );
