...
var AdvancedBox = new function()
{
	...
	var padding = 4;
	/** All object **/
	var objBody, objOverlay, objLightbox, objloadingLink, objBottomNavCloseLink, objZoom, objHoverNav, objOuterImageContainer, objImageContainer, objImage, newObjImage = null;
	...
	/** SlideShow **/
	var objBottomNavPrevLink, objNumberDisplay, objBottomNavNextLink, objBottomNavPlayLink, SlideWait = null;
	...
	**/
	...
	/** Initialize all **/
	Start = function(url /*, width, height, zoomlvl*/ )
	{
		/** At first run, creates the element container **/
		if ( !objOverlay ) { Create(); }	
		...
		var timer = setInterval(function()
		{
			...
			Wait_pass++;
			if ( Wait_pass == 250 ) { clearInterval(timer); Close(); }

			if ( !newObjImage.readyState == "complete" || !newObjImage.complete )
			{
				/** Seems that IE when create an image gives a default width to 28 **/
				if ( newObjImage.width > 100) { clearInterval(timer); Prepare( newObjImage.src, newObjImage.width, newObjImage.height, zoomLevel ); }
			}
			else
			{
				clearInterval(timer);
				Prepare( newObjImage.src, newObjImage.width, newObjImage.height, zoomLevel );
			}
		},10)
	...
	/** Adjust sizes **/
	Prepare = function(CurrentImageSrc, CurrentImageWidth, CurrentImageHeight, CurrentZoomLevel)
	{
alert( 'Prepare Start');
		/** Hide some tags **/
		SelectBoxes( 'hidden' );
		...
		/** Update the image **/
		UpdateImage(CurrentImageSrc, CurrentImageWidth, CurrentImageHeight);

alert( 'Prepare End');
//		ShowImage( );
	},

	UpdateImage = function(CurrentImageSrc, CurrentImageWidth, CurrentImageHeight)
	{
alert( 'UpdateImage Start');
		ElementSetSrc(objLightboxImage, CurrentImageSrc);

		/** Optionally resize the image **/
		var objImagedims = scale_dims(CurrentImageWidth, CurrentImageHeight, MaxWidth, MaxHeight);
		ElementSetWidth(objLightboxImage,  objImagedims.W );
		ElementSetHeight(objLightboxImage, objImagedims.H );
		ElementSetTop(objLightboxImage, ElementGetHeight(objLightboxImage) + ElementGetHeight(objHoverNav) );

		/** Call next step **/
		objLightboxImage.style.visibility = 'hidden';
		UpdateBox( CurrentImageWidth, CurrentImageHeight );
alert( 'UpdateImage End');
		},

	UpdateBox = function( CurrentImageWidth, CurrentImageHeight )
	{
alert( 'UpdateBox Start');
		/** Determine the dimensions of the entire document & current scroll position within the document**/
		var PageSize   = getPageSizes();
		var PageScroll = getPageScroll()

		/** Center the box **/
		ElementSetWidth(objLightbox,  ElementGetWidth (objLightboxImage) + padding*2);
		ElementSetHeight(objLightbox, ElementGetHeight(objLightboxImage) + padding*2 + ElementGetHeight(objHoverNav));

		ElementSetTop(objLightbox,   (PageScroll.YScroll) + ElementGetHeight(objHoverNav));
////////ElementSetTop(objLightbox,   (PageScroll.YScroll + (PageSize.WHeight / 20)));
		ElementSetLeft(objLightbox,  (PageSize.WWidth - ( ElementGetWidth(objLightbox)+ PageSize.WWidth) / 2 ) - borderSize/2 );
		
		/** Call next step **/
		UpdateContainer( CurrentImageWidth, CurrentImageHeight );
alert( 'UpdateBox End');
	},

	UpdateContainer = function( CurrentImageWidth, CurrentImageHeight )
	{
alert( 'UpdateContainer Start');
		/** Update the big container **/
		var objOuterImageContainerdims = scale_dims(CurrentImageWidth, CurrentImageHeight, MaxWidth, MaxHeight);
		ElementSetTop(objOuterImageContainer, ElementGetHeight(objHoverNav));
//		ElementSetWidth(objOuterImageContainer,  objOuterImageContainerdims.W );
//		ElementSetHeight(objOuterImageContainer, objOuterImageContainerdims.H );
		
		BoxScale('outerImageContainer', objOuterImageContainerdims.W, objOuterImageContainerdims.H, ShowImage() );
alert( 'UpdateContainer End');
	},

	/**
	* Code from : http://www.hesido.com/web.php?page=javascriptanimation
	* Updated by MSSTI
	* resize both w & h even if is not set a previous value ;)
	**/
	BoxScale = function( boxElement, boxNewWidth, boxNewHeight, callback )
	{
		var boxObject	= document.getElementById( boxElement );

		var boxWidth	= parseInt( parseFloat( 0 + boxObject.style.width ) );
		var boxNewWidth = parseInt( parseFloat( 0 + boxNewWidth ) );
		var boxHeight	= parseInt( parseFloat( 0 + boxObject.style.height ) );
		var boxNewHeight= parseInt( parseFloat( 0 + boxNewHeight ) );

		DoChange( boxObject, boxWidth, boxNewWidth, boxHeight, boxNewHeight, 25, 100, 0.333 );
	},

	// Width changer with Memory by www.hesido.com
	DoChange = function( elem, startWidth, endWidth, startHeight, endHeight, steps, intervals, powr, callback )
	{
		var actStep = 0;
		elemchange = setInterval( function()
		{
			newH = elem.currentHeight = EaseInOut( startHeight, endHeight, steps, actStep, powr );
			newW = elem.currentWidth = EaseInOut( startWidth, endWidth, steps, actStep, powr );
			elem.style.height = elem.currentHeight.toString()+"px";
			elem.style.width  = elem.currentWidth.toString()+"px";			
		//	alert( 'endWidth=('+endWidth+') newW=('+newW+') endHeight=('+endHeight+') newH=('+newH+') ');
			actStep++;
			if ( actStep > steps )
			{
				alert('BoxScale');
				clearInterval( elemchange );
	...
	{
		var delta = parseInt(maxValue) - parseInt(minValue);
		var stepp = minValue+(Math.pow(((1 / totalSteps) * actualStep), powr) * delta);
		return Math.ceil(stepp)
	},

	/** Finaly show the image using a pseudo light box effect **/
	ShowImage = function( )
	{
alert( 'ShowImage Start');
		objLightboxImage.style.visibility = 'visible';
//		FxFade(objLightboxImage, 100, 0, 100);

		/** Show the image, zoom and close buttons included **/
		ElementShow(objHoverNav);
//		objLightboxImage.style.display = "";
...
alert( 'ShowImage End');
	...
	FxFade = function(elem, start, end, speed, callback)
	{
		elemfade = setInterval(function()
		{
			start = Findend(start, end, speed);
			FxAlpha(elem, start);
alert( 'start=('+start+') end=('+end+') callback=('+callback+') ');
			if(start == end)
			{
				alert('Fin Fade : start=('+start+') end=('+end+')');
				clearInterval( elemfade );
				if ( typeof callback == "function" ) { callback() }
			}
		}, 1);
	},
	
	Findend = function(x, y, speed)
	{
		return x < y ? Math.min(x + speed, y) : Math.max(x - speed, y);
	},


	FxAlpha = function(elem, opacity)
	{
	...
	Loading_Hide = function(e)
	{
		...
		objloadingLink.style.display = "none";
	},
...
//		objLightbox.style.display = objOuterImageContainer.style.display = objloadingLink.style.display = "none";
	},
	...
	Resize = function(/*e*/)
	{
		...
		Close();
		objLightbox.style.display = objOuterImageContainer.style.display = objloadingLink.style.display = "";
		Prepare( CurrImageSrc, RealImgWidth, RealImgHeight, zoomLevel );
	},

	ZoomOutClick = function(e)
	{
		Prepare( objLightboxImage.src, RealImgWidth, RealImgHeight, 10 );
	/*	if (e && typeof(e.stopPropagation) != "undefined") { e.stopPropagation(); } else if (window.event) { window.event.cancelBubble = true; } */
	},
	...
	/**
	* Returns array with : page width, page height, window width, window height, x page scroll, y page scroll values.
	...
	**/
	scale_dims = function(orig_w, orig_h, max_w, max_h)
	{
		var scale = scale_rate(orig_w, orig_h, max_w, max_h);
	...
	scale_rate = function (orig_w, orig_h, max_w, max_h)
	{
    ...
    **/
	...
	/** 
	* Code from : http://www.shawnolson.net/a/1198/hide-select-menus-javascript.html
	* Hide selects/object on page, possible values for action are 'hidden' and 'visible'
	**/
	SelectBoxes = function (action)
	{
		if (action != 'visible') { action = 'hidden'; } 
		if (msie) { for (var S = 0; S < document.forms.length; S++) { for (var R = 0; R < document.forms[S].length; R++) { if (document.forms[S].elements[R].options) { document.forms[S].elements[R].style.visibility = action; } } } }
		var theObjects = document.getElementsByTagName("object");
		for (var i = 0; i < theObjects.length; i++) { theObjects[i].style.visibility = action; }
	};
