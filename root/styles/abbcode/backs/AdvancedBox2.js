...
var AdvancedBox = new function()
{
	...
	**/
	...
	waitFor = function( )
	{
		...
		Wait_pass++;
		if ( Wait_pass == 250 ) { clearTimeout(imgWait); Close(); }
	...
	Initialize = function()
	{
		...
		objOverlay.style.height	= arraySize.PHeight+"px";
		...
		objContainer.setAttribute('id','OC');
...
//		objContainer.onclick = Close;
		...
		objImageContainer.setAttribute('id','OIC');

		/** Create the image here **/
		objImage = document.createElement("img");
		objImage.setAttribute('id','OI');
	...
	Calculate = function ()
	{
		...
		objContainer.style.left	= (!OldStyle ? 0 : arraySize.XScroll )+"px";
		objContainer.style.top	= (!OldStyle ? 0 : arraySize.YScroll )+"px";
	},
	...
	/** Show an image using a pseudo light box effect **/
	XXShowImage = function(url, width, height, zoomlvl)
	{
		/** Resize event handlers **/
		window.onresize = Resize;
...
//ScrollFreeze.on();
		...
		var TEMPobjImageContainer = objImageContainer;
		TEMPobjImageContainer.style.overflow = "";
		objImage.setAttribute('src', url);
alert( 'Size 1 ');
		objImage.style.width = TEMPobjImageContainer.style.width = (RealImgWidth = width)+"px";
		objImage.style.height = TEMPobjImageContainer.style.height = (RealImgHeight = height)+"px";
alert( 'Size 2 ');
		/** Show the image, zoom and close buttons included **/
		objContainer.style.display = "";

		/** Optionally resize the image **/
		objContainer.style.width = objContainer.style.height = TEMPobjImageContainer.style.left = TEMPobjImageContainer.style.top = "";
		var org = 0;
		if (objContainer.offsetWidth > MaxWidth && width)
		{
			org = width;
			...
			width -= objContainer.offsetWidth-MaxWidth+1;
			height = Math.round(height*width/org);
			objImage.style.width = TEMPobjImageContainer.style.width = width+"px";
			objImage.style.height = TEMPobjImageContainer.style.height = height+"px";
		}
		...
		/** Center the box **/
		TEMPobjImageContainer.style.left = Math.floor((objContainer.offsetWidth-TEMPobjImageContainer.offsetWidth)/2-TEMPobjImageContainer.offsetLeft)+"px";
		TEMPobjImageContainer.style.top  = Math.floor((objContainer.offsetHeight-TEMPobjImageContainer.offsetHeight)/2-TEMPobjImageContainer.offsetTop)+"px";
		TEMPobjImageContainer.style.top  = ( msie && msie < 7.0 ) ? (parseInt(TEMPobjImageContainer.style.top )+20)+"px" : TEMPobjImageContainer.style.top ;

		/** Position the Zoom bar **/
		objToolbar.style.left = (parseInt(TEMPobjImageContainer.offsetLeft)   )+"px";
		objToolbar.style.top  = (parseInt(TEMPobjImageContainer.style.top ) + ( msie && msie < 7.0 ? -20 : 10) )+"px";

		/** Update SlideShow **/
		Slide_Show( );
		objImage.style.display = "";
		
		var info_t = TEMPobjImageContainer.style.top;
		var info_l = TEMPobjImageContainer.style.left;
		var info_w = TEMPobjImageContainer.style.width;
		var info_h = TEMPobjImageContainer.style.height;

//		boxScale('OIC', info_w, info_h);
alert( 'Size 3 ');
		/** Optionally zoom in 1:1 **/
		zoomLevel = zoomlvl;
		if (zoomLevel >= 100)
...
/*
	boxScale = function ( boxElement, boxNewWidth, boxNewHeight )
	{
		var boxObject = document.getElementById(boxElement);
		var boxWidth = parseInt(boxObject.style.width);
		doWidthChangeMem( boxObject, boxWidth, boxNewWidth, 25,50, 0.333 );
	},
	
	doWidthChangeMem = function( elem, startWidth, endWidth, steps, intervals, powr )
	{
...
*/	
	Slide_Show = function( PlayOnOff, PlayNumber )
	{
		...
		var curr	= url_clean( objImage.src );
	...
	Resize = function(e)
	{
		...
		Close();
		objOverlay.style.display = objContainer.style.display = objLoading.style.display = "";
	...
	{
		/** Zoom in 1:1 **/
		if (parseInt(objImage.style.width) >= RealImgWidth && parseInt(objImage.style.height) >= RealImgHeight) { return; }
...
NcodeImageResizer.prototype.unScale = function()
{
	...
	{
		case 'AdvancedBox' :
			if ( this.img.className == 'attach_parent' ) { AdvancedBox.Show(this.img.parentNode.href); }
			else { AdvancedBox.Show(this.img.src); }
			break;
		default:
		case 'newwindow':
	...
	for (var pia = 0; pia < posted_images_ary.length; pia++)
	{
		...
		switch ( img.className )
		{
			...
			case 'attach_parent':
				...
				{
					case 'AdvancedBox':
						if ( !ImageResizer  ) { img.onclick = function() { AdvancedBox.Show( this.parentNode.href ); return false; }; ArrayPush( AdvancedBox.SlideShows, img.parentNode.href ); }
						break;
					case 'enlarge':
			...
			case 'attach_me':
				...
				{
					case 'AdvancedBox':
						if ( !ImageResizer ) { img.onclick = function() { AdvancedBox.Show( this.src ); return false; }; ArrayPush( AdvancedBox.SlideShows, img.src ); }
						break;
					case 'enlarge':
