...
function ImageResizerOn( ObjImage )
{
	...
	if ( ImageResizerMode == 'AdvancedBox' ) { 	ObjImage.setAttribute('rel', 'gallery[myset]'); ObjImage.className = ( ObjImage.className ? 'lightview ' + ObjImage.className : 'lightview' ); }
}
	...
	for (var pia = 0; pia < posted_images_ary.length; pia++)
	{
		...
		switch ( img.className )
		{
			...
			case 'hoverbox resize_me':
				...
				{
					case 'AdvancedBox':
						img.className = ( img.className ? 'lightview ' + img.className : 'lightview' ); img.setAttribute('rel', 'gallery[myset]'); img.rel = "gallery[myset]"; img.onclick = function () { Lightview.show({ href: this.src, rel: 'image', options: { width: 800, height: 600 }}); return false; }
						break;
					case 'enlarge':
			...
			case 'attach_parent':
				...
				{
					case 'AdvancedBox':
						if ( !ImageResizer ) { img.className = ( img.className ? 'lightview ' + img.className : 'lightview' ); img.setAttribute('rel', 'gallery[myset]'); img.rel = "gallery[myset]"; img.onclick = function () { Lightview.show({ href: this.parentNode.href, rel: 'image', options: { width: 800, height: 600 }}); return false; } }
						break;
					case 'enlarge':
			...
			case 'attach_me':
				...
				{
					case 'AdvancedBox':
						if ( !ImageResizer ) { img.className = ( img.className ? 'lightview ' + img.className : 'lightview' ); img.setAttribute('rel', 'gallery[myset]'); img.rel = "gallery[myset]"; img.onclick = function () { Lightview.show({ href: this.src, rel: 'image', options: { width: 800, height: 600 }}); return false; } }
						break;
					case 'enlarge':
