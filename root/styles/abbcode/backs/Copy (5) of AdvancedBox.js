...
var AdvancedBox = new function()
{
	...
	var padding = 4;
	/** All object **/
	var objOverlay, objLightbox, objloadingLink, objBottomNavCloseLink, objZoom, objHoverNav, objOuterImageContainer, objImageContainer, objImage, newObjImage = null;
...
//		ElementSetHeight(objOuterImageContainer, objOuterImageContainerdims.H );
		...
		BoxScale('outerImageContainer', objOuterImageContainerdims.W, objOuterImageContainerdims.H, UpdateBox );
	},
...
////////ElementSetTop(objLightbox,   (PageScroll.YScroll + (PageSize.WHeight / 20)));
	...
	**/
	BoxScale  = function( boxElement, boxNewWidth, boxNewHeight, callback )
	{
		...
		DoChange( boxObject, boxWidth, boxNewWidth, boxHeight, boxNewHeight, 25, 100, 0.333, callback );
	},
	...
	DoChange  = function( elem, startWidth, endWidth, startHeight, endHeight, steps, intervals, powr, callback )
	{
		...
		elemchange = setInterval( function()
		{
			...
			elem.style.width  = elem.currentWidth.toString()+"px"; actStep++;

			if ( actStep > steps ) { clearInterval( elemchange ); if ( typeof callback == "function" ) { callback(); } }
		},intervals);
	},
...
//		FxFade(objLightboxImage, 0, 100, 5);
...
/** Common functions - Start **/
	...
	**/
	SelectBoxes = function(action) { if (action != 'visible') { action = 'hidden'; } if (msie) { for (var S = 0; S < document.forms.length; S++) { for (var R = 0; R < document.forms[S].length; R++) { if (document.forms[S].elements[R].options) { document.forms[S].elements[R].style.visibility = action; } } } } var theObjects = document.getElementsByTagName("object"); for (var i = 0; i < theObjects.length; i++) { theObjects[i].style.visibility = action; }; },

	FxFade  = function(elem, start, end, speed, callback) { elemfade = setInterval( function() { start = Findend(start, end, speed); FxAlpha(elem, start); if(start == end) { clearInterval( elemfade ); if ( typeof callback == "function" ) { callback(); } } }, 1); },
