...
var AdvancedBox = new function( )
{
	...
	/** Use sexy looks ? **/
	var animate = false; // Options : true | false
	var elemfade;
	...
	var padding = 4;
	/** Save the "main object" and "image object" for global access **/
	var objOverlay = newObjImage = null;
	...
	Start = function( url )
	{
		...
		**/	
		...
		var timer = window.setInterval( function( )
		{
			...
			Wait_pass++; if ( Wait_pass == 250 ) { window.clearInterval( timer ); Close( ); }
...
////////objOverlay.onclick = function() { Close(); return false; }
	...
	Prepare = function( newObjImage, CurrentZoomLevel )
	{
		...
		if ( !animate ) { ElementInVisible( 'lightboxImage' ); } else { FxFade( 'lightboxImage', 100, 0, animateInDuration ); }
	...
	//	ElementSetTop( 'outerImageContainer', ElementGetHeight( 'hoverNav' ) );
		...
		/** Little trick to prevent image flicker **/
		ElementSetWidth( 'lightboxImage',  1 );
		ElementSetHeight( 'lightboxImage', 1 );
		/** Resize the outerImageContainer very sexy like? **/
		if ( !animate )
		{
			ElementSetWidth( 'outerImageContainer', objOuterImageContainerdims.W );
			ElementSetHeight( 'outerImageContainer', objOuterImageContainerdims.H );
	...
	ZoomInClick = function( e )
	{
		...
		ElementSetHeight( 'lightboxImage', 1 );
		/** Resize the outerImageContainer very sexy like? **/
		if ( !animate ) { ElementSetWidth( 'outerImageContainer', TempWidth ); ElementSetHeight( 'outerImageContainer', TempHeight ); ElementSetTop( 'outerImageContainer', ( ( ( ElementGetHeight( 'lightbox' ) - ElementGetHeight( 'outerImageContainer' ) ) / 2 ) + ElementGetHeight( 'hoverNav' ) ) ); UpdateImage( ); } else { BoxScale( 'outerImageContainer', TempWidth, TempHeight, UpdateImage ); }
	},
...
/** Common functions - Start **/
	...
	**/
	...
	DoChange  = function( elem, startWidth, endWidth, startHeight, endHeight, steps, intervals, powr, callback )
	{
		...
		elem.widthChangeMemInt = window.setInterval( function() { /** The height **/ elem.currentHeight = EaseInOut( startHeight, endHeight, steps, actStep, powr ); elem.style.height = elem.currentHeight+'px'; /** The width **/ elem.currentWidth = EaseInOut( startWidth, endWidth, steps, actStep, powr ); elem.style.width = elem.currentWidth+'px'; /** The top **/ elem.currentTop = ( ( ( ElementGetHeight( 'lightbox' ) - elem.offsetHeight ) / 2 ) + ElementGetHeight( 'hoverNav' ) ); elem.style.top = elem.currentTop+'px'; actStep++; if ( actStep > steps ) { window.clearInterval(elem.widthChangeMemInt); if ( typeof callback == 'function' ) { callback( ); } } },intervals);
	},
	...
	**/
	FxFade  = function( elem, start, end, speed, callback ) { window.clearInterval( elemfade ); elemfade = window.setInterval( function() { start = Findend(start, end, speed); ElementOpacity(elem, start); if(start == end) { window.clearInterval( elemfade ); if ( typeof callback == 'function' ) { callback(); } } }, 1); },
