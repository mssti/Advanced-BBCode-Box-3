...
var AdvancedBox = new function()
{
	...
	// DOM scripting by brothercake -- http://www.brothercake.com/site/resources/scripts/transitions/
	//******************************************************

	//global object
	var isf = { 'clock' : null, 'fade' : true, 'count' : 11 } //var isf = { 'clock' : null, 'fade' : true, 'count' : 1 }
	...
	swapfade = function ()
	{
		...
		if( isf.clock == null )
		{
			...
			else if(typeof isf.obj.style.filter == 'string')
			{
				isf.type = 'ie';
			}
...
//			alert( isf.type );
			...
			if(isf.type != 'none')
			{
				...
				isf.length = parseInt(arguments[2], 10) * 50;

				//create fade resolution argument as 20 steps per transition
				//again, split for the two distrinct transitions
				isf.resolution = parseInt(arguments[2], 10) * 5;

				//start the timer
				isf.clock = setInterval('isfswapfade()', isf.length/isf.resolution);
			}
			//otherwise if opacity is not supported
			else
			{
				//just do the image swap
				objImage.setAttribute('src', isf.src);	//isf.obj.src = isf.src;
	...
	isfswapfade = function()
	{
		...
		{
alert( isf.count +" "+(isf.count < (1 / isf.resolution)) );
			//clear the timer
		...
		switch(isf.type)
		{
			...
			case 'ie7' :
				isf.obj.style.filter = 'alpha(opacity:'+(isf.count * 100)+')';
				break;
...
//			Calculate();
	...
	waitFor = function( )
	{
		...
		if ( !newObjImage.readyState == "complete" || !newObjImage.complete )
		{
			...
			{
				swapfade( objContainer, newObjImage.src, '3' ); //ShowImage( newObjImage.src, newObjImage.width, newObjImage.height, zoomLevel );
			}
			else
			{
	...
	Initialize = function()
	{
		...
		objContainer.style.filter = 'alpha(opacity:'+ojbOpacity+')';
