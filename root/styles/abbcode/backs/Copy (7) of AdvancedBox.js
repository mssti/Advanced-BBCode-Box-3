...
var AdvancedBox = new function()
{
	...
	var animate = true; //= false; //= true; 
	/* Unsigned short greater than 0 (1-100) or unsigned Float greater than 0 (0.1 - 100.0) */
	var animateInDuration = 60;
	var animateOutDuration = 60;
	...
	**/
	DetectBrowser = function()
	{
		...
		browser = browser[2];
		if (browser == "MSIE")			{ var msie = version; }
	...
	Start = function( url )
	{
		...
		**/	
		...
		var timer = setInterval(function()
		{
			...
			Wait_pass++; if ( Wait_pass == 250 ) { clearInterval(timer); Close(); }
			/** Seems that IE when create an image gives a default width to 28 **/
			if ( !newObjImage.readyState == "complete" || !newObjImage.complete ) { if ( newObjImage.width > 100) { clearInterval(timer); Prepare( newObjImage, zoomLevel ); } } 
			else { clearInterval(timer); Prepare( newObjImage, zoomLevel ); }
		},10)
		return false;
	},
	...
	Create = function()
	{
		...
		objOuterImageContainer.style.width = '250px';
		objOuterImageContainer.style.height = '250px';
		...
		for( i = 0, pi = image_url.length; i < pi; i++ ) { preload_image_object.src = "./styles/abbcode/" + image_url[i]; }
	},
	...
	Prepare = function(newObjImage, CurrentZoomLevel)
	{
		...
		/** Update the image **/
		if ( !animate ) { ElementInVisible( 'lightboxImage' ); } 
		else { FxFade( 'lightboxImage', 100, 0, animateInDuration ); }
		...
		ElementSetTop( 'outerImageContainer', ElementGetHeight( 'hoverNav' ));
		ElementSetHeight( 'imageContainer', objOuterImageContainerdims.H );

		ElementSetWidth( 'lightboxImage',  1 );
		ElementSetHeight( 'lightboxImage', 1 );

		if ( !animate  ) { ElementSetWidth( 'outerImageContainer', objOuterImageContainerdims.W); ElementSetHeight( 'outerImageContainer', objOuterImageContainerdims.H ); UpdateImage(); }
		else { BoxScale('outerImageContainer', objOuterImageContainerdims.W, objOuterImageContainerdims.H, UpdateImage ); }
	},
	...
	UpdateImage = function( )
	{
		...
		if ( !animate  ) { ElementVisible( 'lightboxImage' ); }else{ FxFade( 'lightboxImage', 0, 100, animateOutDuration); }
	...
	Slide_Show = function( PlayOnOff, PlayNumber )
	{
		...
		ElementHide( 'nOnText' );
		ElementHide( 'PrevLink' );
		ElementHide( 'NextLink' );
		objBottomNavPlayLink.style.display = "none"; /* ElementHide( 'PlayLink' ); */
		...
		if ( found )
		{
			...
			if ( pos-1 >= 0 ) { ElementShow( 'PrevLink' ); objBottomNavPrevLink.onclick = function() { Start( prev ); return false; }; }

			ElementShow( 'nOnText' );
			ElementSetInnerHTML( 'nOnText', ImageResizerNumberOf.replace(/%1\$s/, pos+1 ).replace(/%2\$s/, last));

			if ( pos+1 < last ) { ElementShow( 'NextLink' ); objBottomNavNextLink.onclick = function() { Start( next ); return false; }; }

			/** Run the SlideShow **/
			if ( last > 1 ) { objBottomNavPlayLink.style.display = ""; /* ElementShow( 'PlayLink' ); */ }
...
/** Common functions - Start **/
	...
	ElementOpacity		= function(element, opacity) { if (window.ActiveXObject) { $(element).style.filter = "alpha(opacity=" + opacity + ")"; }else{ $(element).style.opacity = $(element).style.MozOpacity = $(element).style.KhtmlOpacity = opacity; } },
	ElementSetInnerHTML = function(element,content) { $(element).innerHTML = content; },
	ElementBoxes		= function(action) { return;if (action != 'visible') { action = 'hidden'; } if (IE) { for (var S = 0; S < document.forms.length; S++) { for (var R = 0; R < document.forms[S].length; R++) { if (document.forms[S].elements[R].options) { document.forms[S].elements[R].style.visibility = action; } } } } var theObjects = document.getElementsByTagName("object"); for (var i = 0; i < theObjects.length; i++) { theObjects[i].style.visibility = action; }; },
	
	/** Updated by MSSTI
	* resize both w & h even if is not set a previous value ;) * Code from : http://www.hesido.com/web.php?page=javascriptanimation
	**/
	BoxScale  = function( boxElement, boxNewWidth, boxNewHeight, callback ) { var boxObject	= document.getElementById( boxElement ); var boxWidth	= parseInt( parseFloat( 0 + boxObject.style.width ) ); var boxNewWidth = parseInt( parseFloat( 0 + boxNewWidth ) ); var boxHeight	= parseInt( parseFloat( 0 + boxObject.style.height ) );  var boxNewHeight= parseInt( parseFloat( 0 + boxNewHeight ) ); if ( boxWidth === boxNewWidth && boxHeight === boxNewHeight) { if ( typeof callback == "function" ) { callback( ); } } DoChange( boxObject, boxWidth, boxNewWidth, boxHeight, boxNewHeight, 25, 100, 0.333, callback ); },
	DoChange  = function( elem, startWidth, endWidth, startHeight, endHeight, steps, intervals, powr, callback ) { if (elem.widthChangeMemInt) { window.clearInterval(elem.widthChangeMemInt); } var actStep = 0;  elem.widthChangeMemInt = window.setInterval( function() { elem.currentHeight = EaseInOut( startHeight, endHeight, steps, actStep, powr ); elem.style.height  = elem.currentHeight+"px"; elem.currentWidth  = EaseInOut( startWidth, endWidth, steps, actStep, powr );	elem.style.width   = elem.currentWidth+"px"; actStep++; if ( actStep > steps ) { window.clearInterval(elem.widthChangeMemInt); if ( typeof callback == "function" ) { callback( ); } } },intervals); },
	...
	**/
	FxFade  = function(elem, start, end, speed, callback) { clearInterval( elemfade ); elemfade = setInterval( function() { start = Findend(start, end, speed); ElementOpacity(elem, start); if(start == end) { clearInterval( elemfade ); if ( typeof callback == "function" ) { callback(); } } }, 1); },
	...
	**/
	...
	/**
    * It does a busy-wait loop. * Code from : http://www.faqts.com/knowledge_base/view.phtml/aid/1602
    **/
	// if ( "msie" ){ Pause(250); } else { Pause(100);}Pause = function( numberMillis ) { var now = new Date(); var exitTime = now.getTime() + numberMillis; while (true) { now = new Date(); if (now.getTime() > exitTime) { return; } } };
